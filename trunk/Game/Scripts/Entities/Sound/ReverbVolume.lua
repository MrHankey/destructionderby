  ...
  -- local test=nil + 1;  -- for testing unless lua debugger works
	--if (Preset) then
	  ...
	  if (self.bstarted == 0) then
    	Sound.RegisterWeightedEaxEnvironment(self.Properties.reverbpresetReverbPreset, self.Properties.bFullEffectWhenInside, 0);
    	self.bstarted=1;
    end;
  --end;
    ...
    --System.LogToConsole("RV: Inside ");
    	if (self.fFadeValue ~= fFade) then
    	  ...
    	  --System.LogToConsole("RV: new Fade not old Fade ");
    		Sound.UpdateWeightedEaxEnvironment(self.Properties.reverbpresetReverbPreset, self.fFadeValue);
      ...
      --System.LogToConsole("RV: outside ");
  		local fLocalFade = 1 - (math.sqrt(fDistSq) / self.Properties.OuterRadius);
  	 	...
  	 	if ( fLocalFade > 0) then
    		Sound.UpdateWeightedEaxEnvironment(self.Properties.reverbpresetReverbPreset, fLocalFade);
