	...
	if ((self.Properties.bUsable ~= 0)) then
		...
		if (hits) then
			for i=1,table.getn(hits) do
				...
				if (hit and (not hit.backface) and (hit.distance > 0)) then
					if ((not guiMaterial) or (guiMaterial == hit.material)) then
						local ui = self.ui;
...
function GUI:RenderUI(render)
	...
	if (render) then
		...
		if (not texId) then
			texId = self.ui:CreateRenderTarget();
		end
