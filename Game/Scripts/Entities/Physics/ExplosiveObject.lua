...
--  $Id$
--  $DateTime$
--  Description: Explosive Object Entity
--
----------------------------------------------------------------------------------------------------
--  History:
...
function ExplosiveObject.Client:OnUpdate(frameTime)
	if (self.active) then
		...
		if (self.timer <= 0) then
			...
			-- crossed the decimal unit
			if (math.floor(self.timer+frameTime) > self.timer) then
...
ExplosiveObject.FlowEvents =
{
	Inputs =
	{
		StartTimer = { ExplosiveObject.Event_StartTimer, "bool" },
		StopTimer = { ExplosiveObject.Event_StopTimer, "bool" },
		Hit = { ExplosiveObject.Event_Hit, "bool" },
		Used = { ExplosiveObject.Event_Used, "bool" },
	},
	Outputs =
	{
		StartTimer = "bool",
