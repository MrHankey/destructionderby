...
function RandomSoundVolume:OnPropertyChange()
	...
	if (self.soundName ~= self.Properties.soundName) then -- or self.bEnabled ~= self.Properties.bEnabled) then
...
RandomSoundVolume["Client"] = {
	...
	OnProceedFadeArea = function(self, player, nAreaID, fFade)
	  ...
	  if (self.started == 0) then
	    self.inside = 1;
	    --System.Log("proceed-started"..tostring(self.started));
	    self:Play();
	  end;
...
RandomSoundVolume.FlowEvents =
{
	Inputs =
	{
	  SoundName = { RandomSoundVolume.Event_SoundName, "string" },
