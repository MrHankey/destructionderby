-- TrackPatternManager: defines all the track pattern descriptors
-- Created by Mikko Mononen - 2005
-- Usage:
-- 
...
function TrackPatternManager:OnInit()
	...
	local patSize = 10.0;
	AI.BeginTrackPattern( "test2", AITRACKPAT_VALIDATE_SWEPTSPHERE, 1.0, 2.0 );

		AI.AddPatternNode( "root", 0, 0, -0.5, AITRACKPAT_NODE_ABSOLUTE );
		AI.AddPatternNode( "pt1", 0, (2 / 3) * patSize, 0, AITRACKPAT_NODE_START, "root" );
		AI.AddPatternNode( "pt2", (2 / 3) * patSize, -(1 / 3) * patSize, 0, AITRACKPAT_NODE_START, "root" );
		AI.AddPatternNode( "pt3", -(2 / 3) * patSize, -(1 / 3) * patSize, 0, AITRACKPAT_NODE_START, "root" );
	...
	local innerSize = 2.5;
	local midSize = 10.0;
	local outerSize = 25.0;
--	AI.BeginTrackPattern( "test", AITRACKPAT_VALIDATE_SWEPTSPHERE+AITRACKPAT_ALIGN_RANDOM+AITRACKPAT_ALIGN_LEVEL_TO_TARGET, 2.0, 0.35, 0.4, 0.0, 0.1, 0.5, {x=10, y=10, z=90} );
	AI.BeginTrackPattern( "test", AITRACKPAT_VALIDATE_SWEPTSPHERE+AITRACKPAT_ALIGN_LEVEL_TO_TARGET, 2.0, 3.0, 0.35, 0.4, 0.0, 0.1, 0.5 );
	...
	for aa = 1, 2, 1 do
		AI.BeginTrackPattern( "melee_circle"..aa, AITRACKPAT_VALIDATE_SWEPTSPHERE+AITRACKPAT_ALIGN_RANDOM, 0.5, 2, 0.35, 0.4, 0.0, 0.1, -1, {x=5, y=5, z=90} );
			...
			-- add link to both ways, so that it is possible to stick at "shadow"
			for i = 1, pts, 1 do
				local n = i + 1;
				local p = i - 1;
				if (n > pts) then n = 1 end;
				if (p < 1) then p = pts end;
				AI.AddPatternBranch( "p"..i, AITRACKPAT_CHOOSE_MOST_EXPOSED, "p"..n );
				AI.AddPatternBranch( "p"..i, AITRACKPAT_CHOOSE_MOST_EXPOSED, "p"..p );
--				AI.AddPatternBranch( "root", AITRACKPAT_CHOOSE_MOST_EXPOSED, "p"..i );
			end
		AI.EndTrackPattern();
	end

	AI.BeginTrackPattern( "test_buffa_attack", AITRACKPAT_VALIDATE_SWEPTSPHERE+AITRACKPAT_ALIGN_TARGET_DIR, 0.5, 2.0, 0.35, 0.4, 0.0, 0.1, 1 );
		...
		AI.AddPatternNode( "p4", 0, 5.0, 1.5, AITRACKPAT_NODE_ABSOLUTE+AITRACKPAT_NODE_SIGNAL, "", 1000 );
	...
	AI.BeginTrackPattern( "test_buffa_attack2", AITRACKPAT_VALIDATE_SWEPTSPHERE+AITRACKPAT_ALIGN_TARGET_DIR, 0.5, 2.0, 0.35, 0.4, 0.0, 0.1, 1 );
	...
	AI.BeginTrackPattern( "melee_circle_flyby1", AITRACKPAT_VALIDATE_SWEPTSPHERE+AITRACKPAT_ALIGN_TARGET_DIR+AITRACKPAT_ALIGN_RANDOM, 0.5, 2.0, 0.35, 0.4, 0.0, 0.1, 1, {x=0, y=60, z=0} );
