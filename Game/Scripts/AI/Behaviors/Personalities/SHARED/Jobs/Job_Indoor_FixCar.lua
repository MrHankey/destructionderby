...
----	AIANCHOR_RANDOM_TALK if AI finds one of these will try to initiate conversation
-- Created 2002-12-09 Amanda
--------------------------
AIBehaviour.Job_Indoor_FixCar = {
	Name = "Job_Indoor_FixCar",
	JOB = 1,
	AnimTable ={
	...
	FIND_ANCHOR = function (self,entity)
		...
		if (entity.AI_FoundObject) then
			entity:SelectPipe(0,"anchor_indoorCar",entity.AI_FoundObject);
	...
	DECISION_POINT = function( self,entity , sender)
		...
		if ( rnd <3) then 
			entity.cnt.AnimationSystemEnabled = 1;
			entity:SelectPipe(0,"get_toolbox_indoor",entity.AI_FoundObject);
		-- or bored enough to take a break	
		elseif (rnd <5) then		
			entity.cnt.AnimationSystemEnabled = 1;
			entity:InsertSubpipe(0,"pause");
			local boredAnchor = AI_BoredManager:FindAnchor(entity,AI_BoredManager.AIBORED_INDOOR_GRUNT,20);
			if (boredAnchor) then
