...
float m_fVehicleViewPitch = 0.0f;
float m_fVehicleViewYaw = 0.0f;
...
void CPlayerView::Process(SViewParams &viewParams)
{
	...
	//gEnv->pRenderer->Draw2dLabel( 100, 50, 2, white, false, "yup" );
...
void CPlayerView::ViewFirstPerson(SViewParams &viewParams)
{
        ...
        // [WR] Set initial weapon angle offset to match a bit of the cursor
		if (CHUD *pHud = g_pGame->GetHUD())
		{
			if (CHUDCrosshair *pCH = pHud->GetCrosshair())
			{
				Vec3 vCursorPos;
                //pCH->IRGetCursorPos(vCursorPos);
				const float fMoveHoriz = g_pGameCVars->cl_fov;
				const float fMoveVert = g_pGameCVars->cl_fov;
				weaponAngleOffset.z = -(vCursorPos.x * fMoveHoriz - fMoveHoriz*0.5f);
				weaponAngleOffset.x = -(vCursorPos.y * fMoveVert - fMoveVert*0.5f);
			}
		}
		// [/WR]
...
{
    if ( !g_pGame->GetIGameFramework()->GetISystem()->IsDedicated() )
    {
	    if (m_in.pVehicle && !g_pGame->GetHUD()->GetInMenu() )
	    {
            Vec3 lookAtPos = m_in.pVehicle->GetEntity()->GetWorldPos();
            lookAtPos.z += 1.5f;
            Ang3 angles = m_in.pVehicle->GetEntity()->GetWorldAngles();
            Vec3 camPos;        
            
            int iCenterX = gEnv->pRenderer->GetWidth()/2;
            int iCenterY = gEnv->pRenderer->GetHeight()/2;
            float fMouseX, fMouseY;

            gEnv->pHardwareMouse->GetHardwareMousePosition ( &fMouseX, &fMouseY );
            gEnv->pHardwareMouse->SetHardwareMousePosition ( iCenterX, iCenterY );

            m_fVehicleViewPitch += (fMouseY - iCenterY) * 0.3f * viewParams.frameTime; 
	        m_fVehicleViewPitch = CLAMP(m_fVehicleViewPitch, -DEG2RAD(15.0f), DEG2RAD(80.0f)); 
	        m_fVehicleViewYaw += (fMouseX - iCenterX) * 0.3f * viewParams.frameTime; 
            float distance  = 10.0f;
            
            camPos.z = (float)sin(m_fVehicleViewPitch) * distance; 
            camPos.y = (float)cos(m_fVehicleViewYaw) * (float)cos(m_fVehicleViewPitch) * distance; 
            camPos.x = (float)sin(m_fVehicleViewYaw) * (float)cos(m_fVehicleViewPitch) * distance;

            camPos += lookAtPos;

            static ray_hit hit;
            IPhysicalEntity* pSkipEntities[10];
	        int nSkip = nSkip = m_in.pVehicle->GetSkipEntities(pSkipEntities, 10);

            Vec3 dir = camPos - lookAtPos;
	        primitives::sphere sphere;
	        sphere.center = lookAtPos;
	        sphere.r = 0.3f;
	        ...
	        {
		        camPos = lookAtPos + (hitDist * dir.GetNormalizedSafe());
        		
		        if(camPos.z - lookAtPos.z < minHeightDiff)
		        {
			        ...
			        float raiseDist = minHeightDiff - (camPos.z - lookAtPos.z) - wallSafeDistance;
			        if(newHitDist != 0)
			        {
			        ...
			        camPos.z += raiseDist;
			        lookAtPos.z += raiseDist*0.8f;
		        }
	        }

            const Vec3 forward = (lookAtPos-camPos).GetNormalizedSafe(Vec3Constants<float>::fVec3_OneY);
...
void CPlayerView::ViewDeathCamTarget(SViewParams &viewParams)
{
	...
	float scale = 1.0f;
	float minDistSquared = (g_pGameCVars->g_deathCamMinZoomDistance * g_pGameCVars->g_deathCamMinZoomDistance);
	float maxDistSquared = (g_pGameCVars->g_deathCamMaxZoomDistance * g_pGameCVars->g_deathCamMaxZoomDistance);
	if(timeNow - m_in.deathTime > 1.0f && distSq > minDistSquared)
	{
		// fov settings are read from the cvars now
		float fov = CLAMP(g_pGameCVars->g_deathCamMaxZoomFOV, 0.1f, 1.0f);
		
		scale = 1.0f - (1.0f - fov) * ((distSq - minDistSquared)/(maxDistSquared - minDistSquared));
		scale = CLAMP(scale, 0.1f, 1.0f);
	}

	Interpolate(oldCamPos, camPos, 5.0f, viewParams.frameTime);
	Interpolate(oldTargetPos, targetPos, 5.0f, viewParams.frameTime);
	Interpolate(oldFOVScale, scale, 1.0f, viewParams.frameTime);
